...
#### Handshake  
  ...
  - Valid is sticky, must remain asserted until destination accepts information
...
01-Ex-Okay  
10-SLVERR  
  - when there is unsupported transfer size is attempted, or unsupported burst type.  
  - when you write on read only space
  - When you are readinig from write only space(google it)
11-DECERR generated by interconnect/address decoder block  
  - When the requrested address does not exist i.e it tells the given address does not belong to any of the slaves available
  - no slave exists for this address
  - taget slave is disabled(turned off)
...
### QoS AxQOS (Quality of service)(prioritize transactions)  
  x0 -> lowest priority  
  xf -> highest priority  
Sometimes CPU require memory access that are more important than GPU or any other system

### AXI channel dependancies
- wlast must complete before bvalid is asserted i.e $rose(wlast) |->##[1:$] $rose(bvalid)
  - Master must send all data before a write response can by seen by master i.e $rose(wlast) |->##1 $rose(bresp)
...
interleaving -> master's job because it's related with burst 
out of order -> slave's job because it's related with response which are generated by salves
Interleaving concept is related with burst wdata(for AXI3) and rdata  
...
- Note: In AXI4 we don't have WID to keep track of xtn hence interleaving for write xtn is not supported
...
AR   A  B   address read
RID  1  0    
R    BL AL   rdata and rresp on same channel
...
Note: There won't be any data loss in unaligned txn because we use strobe signal to indicate valid data   
